...
- [📋 목차](#목차)
- [AsyncTask란?](#asynctask란)
  - [핵심 특징](#핵심-특징)
  - [AsyncTask vs Step 비교](#asynctask-vs-step-비교)
- [왜 필요한가?](#왜-필요한가)
  - [문제 상황](#문제-상황)
  - [AsyncTasks로 해결](#asynctasks로-해결)
  - [이점](#이점)
- [실행 타이밍 (언제 실행되는가?)](#실행-타이밍-언제-실행되는가)
  - [실행 흐름](#실행-흐름)
  - [타임라인 예시](#타임라인-예시)
  - [중요한 규칙](#중요한-규칙)
- [AsyncTask 실행 조건 (중요!)](#asynctask-실행-조건-중요)
  - [✅ AsyncTask가 실행되는 경우](#asynctask가-실행되는-경우)
  - [❌ AsyncTask가 실행되지 않는 경우](#asynctask가-실행되지-않는-경우)
  - [🎯 핵심 규칙](#핵심-규칙)
- [기본 사용법](#기본-사용법)
  - [1. 프로젝트 구조](#1-프로젝트-구조)
  - [2. Feature 정의](#2-feature-정의)
  - [3. AsyncTask 파일 작성](#3-asynctask-파일-작성)
  - [4. 실행 순서](#4-실행-순서)
- [실무 사용 사례](#실무-사용-사례)
  - [사례 1: 주문 완료 후 이메일/알림](#사례-1-주문-완료-후-이메일알림)
  - [사례 2: 회원 가입 후 환영 이메일](#사례-2-회원-가입-후-환영-이메일)
  - [사례 3: 분석 이벤트 발송](#사례-3-분석-이벤트-발송)
  - [사례 4: 외부 시스템 연동](#사례-4-외부-시스템-연동)
  - [사례 5: 로그 수집](#사례-5-로그-수집)
  - [사례 6: 캐시 무효화](#사례-6-캐시-무효화)
  - [사례 7: Webhook 발송](#사례-7-webhook-발송)
- [Context와의 관계](#context와의-관계)
  - [Context란?](#context란)
  - [Steps에서 Context 생성](#steps에서-context-생성)
  - [AsyncTasks에서 Context 사용](#asynctasks에서-context-사용)
  - [중요한 차이점](#중요한-차이점)
- [Best Practices](#best-practices)
  - [1. 필요한 데이터는 Steps에서 미리 Context에 저장](#1-필요한-데이터는-steps에서-미리-context에-저장)
  - [2. AsyncTask는 멱등성(Idempotent)을 고려](#2-asynctask는-멱등성idempotent을-고려)
  - [3. 에러 처리는 각 AsyncTask에서 개별적으로](#3-에러-처리는-각-asynctask에서-개별적으로)
  - [4. 순서가 중요하다면 파일명에 숫자 사용](#4-순서가-중요하다면-파일명에-숫자-사용)
  - [5. 긴 작업은 큐 시스템 사용 고려](#5-긴-작업은-큐-시스템-사용-고려)
- [다른 솔루션과의 비교](#다른-솔루션과의-비교)
  - [vs Express.js 수동 처리](#vs-expressjs-수동-처리)
  - [vs Bull Queue / Agenda](#vs-bull-queue-agenda)
  - [vs Lambda / Cloud Functions](#vs-lambda-cloud-functions)
- [FAQ](#faq)
  - [Q1. AsyncTask는 병렬 실행되나요?](#q1-asynctask는-병렬-실행되나요)
  - [Q2. AsyncTask에서 에러가 발생하면?](#q2-asynctask에서-에러가-발생하면)
  - [Q3. AsyncTask 실행 여부를 확인할 수 있나요?](#q3-asynctask-실행-여부를-확인할-수-있나요)
  - [Q4. AsyncTask에서 다른 AsyncTask의 결과를 사용할 수 있나요?](#q4-asynctask에서-다른-asynctask의-결과를-사용할-수-있나요)
  - [Q5. AsyncTask를 선택적으로 실행할 수 있나요?](#q5-asynctask를-선택적으로-실행할-수-있나요)
  - [Q6. Steps와 AsyncTasks의 차이가 뭔가요?](#q6-steps와-asynctasks의-차이가-뭔가요)
  - [Q7. AsyncTask가 너무 오래 걸리면 어떻게 하나요?](#q7-asynctask가-너무-오래-걸리면-어떻게-하나요)
  - [Q8. AsyncTask는 재시도되나요?](#q8-asynctask는-재시도되나요)
- [마무리](#마무리)
