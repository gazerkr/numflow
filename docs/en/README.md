# Numflow Framework Documentation

Express.js-compatible high-performance Node.js web framework

## Table of Contents

- [Documentation Structure](#-documentation-structure)
- [Project Goals](#-project-goals)
- [Documentation Guide](#-documentation-guide)
- [Last Updated](#-last-updated)

## 📚 Documentation Structure

### Getting Started

- **[Getting Started](./getting-started/README.md)** - User guide (recommended for beginners)
  - [Installation & First Application](./getting-started/first-app.md)
  - [Routing](./getting-started/routing.md)
  - [Request/Response](./getting-started/request-response.md)
  - [Middleware](./getting-started/middleware.md)
  - [Error Handling](./getting-started/error-handling.md)
  - [Feature-First](./getting-started/feature-first.md)
  - [Async Tasks](./getting-started/async-tasks.md)
  - [Express Migration](./getting-started/migration-from-express.md)
  - [Project Structure](./getting-started/project-structure.md)
  - [Debug Mode](./getting-started/debug-mode.md)
  - [TypeScript](./getting-started/typescript.md)

### API Reference

- **[API Documentation](./api/)** - Detailed API reference
  - [Application](./api/application.md)
  - [Router](./api/router.md)
  - [Request](./api/request.md)
  - [Response](./api/response.md)
  - [Feature](./api/feature.md)

### Advanced Guides

- **[ARCHITECTURE.md](./ARCHITECTURE.md)** - System architecture design
- **[PERFORMANCE.md](./PERFORMANCE.md)** - Performance goals and benchmark results
- **[COMPATIBILITY.md](./COMPATIBILITY.md)** - Express compatibility matrix

## 🎯 Project Goals

1. **Full Express Compatibility** - Use existing Express middleware/plugins as-is
2. **High Performance** - **228% faster than Express** on average (3.3x performance)
3. **TypeScript Native** - Perfect type inference support
4. **Centralized Error Handling** - Automatic error handling without try-catch

### Latest Benchmark Results (2025-11-15)

- **vs Express**: +228% performance improvement (avg 64,634 req/s vs 19,694 req/s)
- **vs Fastify**: -19% (Fastify is slightly faster, avg 80,188 req/s)
- **POST requests**: Surpasses Fastify +12% (57,872 vs 51,664 req/s)
- **Feature-First overhead**: 0.70% (negligible)
- **Latency**: 1.09ms (1/4 of Express)

## 📖 Documentation Guide

### New to Numflow?

1. **[Getting Started](./getting-started/README.md)** - From installation to first app
2. **[First Application](./getting-started/first-app.md)** - Create Hello World
3. **[Routing](./getting-started/routing.md)** - Learn basic routing

### Migrating from Express

- **[Express Migration](./getting-started/migration-from-express.md)** - Step-by-step migration guide
- **[Compatibility Guide](./COMPATIBILITY.md)** - Express middleware compatibility info

### Advanced Users

- **[Feature-First](./getting-started/feature-first.md)** - Advanced architecture pattern
- **[Architecture](./ARCHITECTURE.md)** - System design principles
- **[Performance Guide](./PERFORMANCE.md)** - Performance optimization and benchmarks

## 📅 Last Updated

- 2025-11-15: Latest benchmark results and test pass rates added
- 2025-11-11: Documentation structure reorganization (production ready)
